...
def amortized(forward, bs=100):
    def _forward(self, x):
        if x.shape[0] > bs:
            n_batches = np.ceil(x.shape[0] / bs)
            outputs = []
            for i in range(int(n_batches)):
                outputs.append(forward(self, x[i*bs : (i + 1)*bs]))
...
class ResNet(nn.Module):
    ...
    # TODO: Take care of relu before cosine
    def __init__(self, block, layers, classifier=False,
                 num_classes=1000, dropout=0.0):
    ...
    def cosine_cls_fn(self, x):
        return self.cosine_sim(x.unsqueeze(-1), self.weight) * self.scaler
